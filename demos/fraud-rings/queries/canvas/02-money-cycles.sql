...
FROM ag_catalog.cypher('fraud_rings', $$
  MATCH (a:Client)-[:USED_DEVICE]->(d:Device)<-[:USED_DEVICE]-(b:Client), (a)-[r:PAID]->(b) WHERE a.id <> b.id RETURN DISTINCT a._key, label(a), coalesce(a.name, a.id, a._key), b._key, label(b), coalesce(b.name, b.id, b._key), type(r), r._key, coalesce(r.amount, a.amount, b.amount, 0) LIMIT 500
