...
FROM ag_catalog.cypher('edge_fleet', $$
  MATCH (a:Device)-[r:DEPENDS_ON]->(b:Device) RETURN DISTINCT a._key, label(a), coalesce(a.name, a.id, a._key), b._key, label(b), coalesce(b.name, b.id, b._key), type(r), r._key, coalesce(r.amount, a.amount, b.amount, 0) LIMIT 500
