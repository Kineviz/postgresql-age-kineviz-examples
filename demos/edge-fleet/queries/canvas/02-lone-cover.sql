...
FROM ag_catalog.cypher('edge_fleet', $$
  MATCH (t:Technician)-[:COVERS]->(b:Site) WITH b, count(DISTINCT t) AS n WHERE n = 1 MATCH (a:Technician)-[r:COVERS]->(b) RETURN DISTINCT a._key, label(a), coalesce(a.name, a.id, a._key), b._key, label(b), coalesce(b.name, b.id, b._key), type(r), r._key, coalesce(r.amount, a.amount, b.amount, 0) LIMIT 500
